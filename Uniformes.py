...
class Uniformes(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        ...
        self.logo_label = customtkinter.CTkLabel(self.sidebar_frame, text="Uniformes", font=customtkinter.CTkFont(size=20, weight="bold"))
        ...
        def crear_ventana_entrada():
            ...
            tk.Label(ventana_entrada, text="Id Uniforme: ").grid(row=0)
            nombre_entry = tk.Entry(ventana_entrada)
            nombre_entry.grid(row=1, column=1)
            tk.Label(ventana_entrada, text="Id Colegio: ").grid(row=1)
            apellido_entry = tk.Entry(ventana_entrada)
            apellido_entry.grid(row=2, column=1)
            tk.Label(ventana_entrada, text="Tipo Uniforme: ").grid(row=2)
            ...
            boton_anadir = tk.Button(ventana_entrada, text="Añadir", command=lambda: insertUniform(id_entry.get(), nombre_entry.get(), apellido_entry.get(), telefono_entry.get()))            
        ...
        def crear_ventana_borrar():
            ...
            boton_borrar = tk.Button(ventana_borrar, text="Borrar", command=lambda: deleteUniform(id_entry.get()))            
        ...
        def crear_ventana_update():
            ...
            label2 = tk.Label(new_window, text="ID Uniforme")
            entry2 = tk.Entry(new_window)
            label2.grid(row=1, column=0)
            entry2.grid(row=1, column=1)

            label3 = tk.Label(new_window, text="ID Colegio")
            entry3 = tk.Entry(new_window)
            label3.grid(row=2, column=0)
            entry3.grid(row=2, column=1)

            label4 = tk.Label(new_window, text="Tipo Uniforme")
            ...
            update_button = tk.Button(new_window, text="Actualizar", command=lambda: UpdateUniform(entry1.get(), entry2.get(), entry3.get(), entry4.get(), entry5.get()))
            update_button.grid(row=5, column=1)  # Colocar el botón debajo de las entradas

            new_window.mainloop()
        self.sidebar_button_3 = customtkinter.CTkButton(self.sidebar_frame, command=crear_ventana_entrada, text="Agregar Uniforme")
        self.sidebar_button_3.grid(row=3, column=0, padx=20, pady=10)
        self.sidebar_button_4 = customtkinter.CTkButton(self.sidebar_frame, command=crear_ventana_borrar, text="Borrar Uniforme")
        self.sidebar_button_4.grid(row=4, column=0, padx=20, pady=10)
        self.sidebar_button_5 = customtkinter.CTkButton(self.sidebar_frame, command=crear_ventana_update, text="Actualizar Uniforme")
        ...
        self.main_button_1 = customtkinter.CTkButton(text="Consultar",master=self, fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"), command=lambda: self.mostrar_tabla_event("uniforme"))
